...
#ifndef COMMON_ENDIAN_HPP
#define COMMON_ENDIAN_HPP

#include "src/common/system.hpp"
#include "src/common/types.hpp"

// Sanity check
#if !defined(DECEMVIRATE_LITTLE_ENDIAN) && !defined(DECEMVIRATE_BIG_ENDIAN)
...
#if !defined(DECEMVIRATE_NEED_ALIGNMENT)
...
// use software fallback by loading each byte explicitly
	...
	#if defined(DECEMVIRATE_LITTLE_ENDIAN)
	...
	#elif defined(DECEMVIRATE_BIG_ENDIAN)
...
// if the unaligned load and the byteswap take a lot instructions its better to directly read and invert
	#if defined(DECEMVIRATE_NEED_ALIGNMENT) && !defined(__mips__)
	...
	#endif // if defined(DECEMVIRATE_NEED_ALIGNMENT)
...
#endif // if defined(DECEMVIRATE_LITTLE_ENDIAN)

#endif // COMMON_ENDIAN_HPP
