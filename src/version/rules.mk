...
gitstamp:
	@-$(eval $@_REVLINE := $(shell cd $(srcdir); git describe --tags --long --match desc/\* 2>/dev/null | cut -d '/' -f 2- | sed -e 's/\(.*\)-\([^-]*\)-\([^-]*\)/\1+\2.\3/'))
...
VERSION_FLAGS  =
VERSION_FLAGS += $(shell if test x$(VERSION_REVDESC) != "x"; then echo "-DDECEMVIRATE_REVDESC=\\\"$(VERSION_REVDESC)\\\""; fi)
VERSION_FLAGS += $(shell if test x$(VERSION_REV) != "x"; then echo "-DDECEMVIRATE_REV=\\\"$(VERSION_REV)\\\""; fi)
VERSION_FLAGS += $(shell if test x$(VERSION_DATE) != "x"; then echo "-DDECEMVIRATE_BUILDDATE=\\\"$(VERSION_DATE)\\\""; fi)
...
src/version/version.lo: $(GITSTAMP_FILE) src/version/version.cpp src/version/version.hpp
