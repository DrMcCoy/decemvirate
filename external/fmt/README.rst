...
* Small code size both in terms of source code (the minimum configuration
  consists of just three header files, ``core.hpp``, ``format.hpp`` and
  ``format-inl.hpp``) and compiled code. See `Compile time and code bloat`_.
...
.. code::
    ...
    std::string s = format(fmt("{2}"), 42);
                    ^
    include/fmt/core.hpp:778:19: note: non-constexpr function 'on_error' cannot be used in a constant expression
        ErrorHandler::on_error(message);
                      ^
    include/fmt/format.hpp:2226:16: note: in call to '&checker.context_->on_error(&"argument index out of range"[0])'
