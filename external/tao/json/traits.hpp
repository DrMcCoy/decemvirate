...
namespace tao::json
{
   ...
   {
      template< template< typename... > class Traits >
      static void assign( basic_value< Traits >& v, const std::vector< T >& o )
      {
         v = std::vector< basic_value< Traits > >( o.begin(), o.end() );
      }

      template< template< typename... > class Traits >
      [[nodiscard]] static std::vector< T > as( const basic_value< Traits >& v )
      {
         if( v == null ) {
            return std::vector< T >();
         }

         const auto xv = v.get_array();

         std::vector< T > o;
         for( const auto& x : v.get_array() ) {
            o.emplace_back(x.template as< T >());
         }
